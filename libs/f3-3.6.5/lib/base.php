...
final class Base extends Prefab implements ArrayAccess {
	...
	*	@param $tags string
	**/
	function clean($arg,$tags=NULL,$html=false,$extra=false) {
		if (isset($arg) && !empty($arg)) {
			// Main clean
			if ($html === true) {
				$string = htmlentities(strip_tags(trim($arg)), ENT_QUOTES, 'UTF-8');
			}
			else {
				$string = htmlspecialchars(strip_tags(trim($arg)), ENT_QUOTES, 'UTF-8');
			}
